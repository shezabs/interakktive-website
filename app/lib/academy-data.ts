...
export const academyCourses: AcademyCourse[] = [
  ...
  },
  {
    ...
    lessons: [
      ...
      { id: 'cipher-regime-engine', courseId: 'level-11-cipher-pro-mastery', title: 'The 3-Regime Engine', subtitle: 'TREND · RANGE · VOLATILE — and the forecaster on top.', description: 'The start of the Regime Engine arc. CIPHER classifies every bar into one of three regimes via a three-score race: trend_pct (ADX × 2.5, cap 100) / volatile_pct (ramps 0→100 between 1.5× and 2.0× ATR/ATR_slow) / range_pct (the residual). Tallest wins. Ties resolve TREND > VOLATILE > RANGE. The Groundbreaking Concept — "The Regime as Forecaster, Not Observer" — introduces the Regime Transition Predictor (RTP), the probability engine that runs on top of classification: it tracks how long each regime lasts on your chart, compares to the 30-deep rolling historical average, and uses a sigmoid curve to predict when the regime is about to flip. The guidance cell outputs plain-English forecasts — INTACT / HOLDING / CAUTIOUS / X FORMING / SHIFTING TO X — with no raw numbers shown. The five downstream effects of regime: header state, action cell string gating, transition guidance, Risk Envelope band widening (+10% in VOLATILE only), alert JSON payload. The No Hysteresis design choice and why RTP guidance — not bar-lock — is the stability layer operators should trust. Three regime playbooks (TREND / RANGE / VOLATILE) with entry, stop, size, and avoid rules for each. Six common regime-reading mistakes. Earns the Regime Reader Operator certificate.', estimatedMinutes: 42, isFree: false, totalSections: 17, quizPassThreshold: 66 },
      { id: 'cipher-regime-transitions', courseId: 'level-11-cipher-pro-mastery', title: 'Regime Transitions and Hysteresis', subtitle: 'Act before the flip, not after.', description: 'The second lesson of the Regime Engine arc. The Three-Stage Warning System: INTACT (prob < 50%), X FORMING (50-75%), SHIFTING TO X (> 75%). Why CIPHER uses no hysteresis and why the RTP probability layer is the stability mechanism instead. Deep dive into the sigmoid formula: at 50% of avg duration prob is ~25%, at avg prob is ~60%, at 1.4x avg prob is ~82%, capped at 95%. The 30-deep rolling duration arrays and the six-counter transition matrix. Timeframe character and VOLATILE as the fastest-escalating regime. FORMING playbook (partial exit + stop tighten) and SHIFTING playbook (full exit before cell 2 confirms). Full Command Center multi-row convergence. Six transition-timing mistakes. Earns the Transition Timing Operator certificate.', estimatedMinutes: 45, isFree: false, totalSections: 17, quizPassThreshold: 66 },
      { id: 'cipher-executive-summary', courseId: 'level-11-cipher-pro-mastery', title: "The Executive Summary — CIPHER's Priority Waterfall", subtitle: '17 conditions. One output. The header has already resolved the conflict.', description: 'The third lesson of the Regime Engine arc. The Groundbreaking Concept — The Single Most Urgent Truth — reveals how the always-on CIPHER PRO header row evaluates 17 conditions every bar via a strict priority waterfall and outputs the single highest-priority true condition. The four-color urgency grammar: teal (go), amber (caution), bright red danger (#FF1744), magenta volatile/structural (#EF5350), plus cyan for BUILDING. Full anatomy of all 17 header action states in exact priority order from Pine line 3089. The header state cell (col 2): BULL TREND / BEAR TREND / VOLATILE / RANGING and what drives each. Regime gating: which states can only fire in TREND, RANGE, or VOLATILE. The RIDE IT deep dive: three simultaneous conditions required. Conflict resolution guarantee: when rows show mixed colors, the header has already resolved the conflict via cascade. TREND HOLLOWING vs TREND CURVING distinction. FADING vs REDUCE SIZE — two danger colors, two crises. The four-step operator read discipline. The WAIT fallback state. Six common header-reading mistakes. Earns the Narrative Reader Operator certificate.', estimatedMinutes: 48, isFree: false, totalSections: 17, quizPassThreshold: 66 },
      { id: 'cipher-regime-sizing', courseId: 'level-11-cipher-pro-mastery', title: 'Regime-Based Position Sizing', subtitle: 'The bands move with the regime. Your size should too.', description: 'CIPHER\'s Risk Envelope is a live position sizing engine. This lesson teaches the four risk zones (SAFE/WATCH/CAUTION/DANGER), how band scale adapts across RANGE, TREND, and VOLATILE regimes, directional asymmetry, dwell phases, the Mean Reversion Score, and the complete sizing playbook. Earn the Sizing Operator certificate.', estimatedMinutes: 45, isFree: false, totalSections: 17, quizPassThreshold: 66 },
      { id: 'cipher-signal-philosophy', courseId: 'level-11-cipher-pro-mastery', title: 'The Signal Philosophy', subtitle: 'Every signal you see survived a five-gate qualification pipeline.', description: 'CIPHER does not fire signals freely — it qualifies them. This lesson teaches the dual signal origins (PX and TS), the Signal Engine mode and Direction filter, the 4-factor conviction scoring system, the Strong Only gate, the 13-level context tag waterfall, and Last Signal freshness states. Earn the Signal Architect certificate.', estimatedMinutes: 45, isFree: false, totalSections: 17, quizPassThreshold: 66 },
      { id: 'cipher-pulse-factor', courseId: 'level-11-cipher-pro-mastery', title: 'The 5-Layer Pulse Factor', subtitle: 'You set one number. CIPHER reads your context and tunes it for you.', description: 'The first lesson of the Signal Engine arc. The Groundbreaking Concept — The Self-Calibrating Heartbeat — reveals that your i_pulse_factor input is just the seed. Five hidden layers compound on top: Timeframe (Weekly 0.70, 1H/4H/D 0.85, Scalp/Intra 1.00), Asset Class (Forex/Commodity 0.80, Crypto 0.90, Index 0.95, Stock 1.00), Volatility (clamp 0.80-1.30 by vol_ratio), ADX/Trend Strength (strong 0.85, mid 1.00, chop 1.15), and Active Preset (Scalper 0.80, default 1.00, Swing 1.20, Sniper 1.30). Full anatomy of the Pulse line build: Cipher Flow anchor + ATR distance + Supertrend ratchet + SMA smoothing. Pulse Smoothing input deep dive (default 3, lag-vs-stability tradeoff). The maturity lifecycle: FRESH (0-5b) / YOUNG (5-20b) / ESTABLISHED (20-50b) / MATURE (50+b) and why MATURE-flips are structurally significant. The choppy flag: third-most-recent flip within 20 bars triggers SIGNALS UNRELIABLE. Slope verdicts: STEEPENING / STEADY / FLATTENING. Reading the Pulse Command Center row: 5 proximity states (FLIPPED / STRETCHED / HOLDING / CLOSE / VERY CLOSE) and 6 guidance verdicts. Six common Pulse Factor mistakes. Earns the Pulse Factor Operator certificate.', estimatedMinutes: 50, isFree: false, totalSections: 17, quizPassThreshold: 66 },
      { id: 'cipher-px-pipeline', courseId: 'level-11-cipher-pro-mastery', title: 'PX Filter Pipeline', subtitle: 'Pulse flips happen often. Most are filtered. The few that survive are PX signals.', description: 'Lesson 2 of the Signal Engine arc. The Groundbreaking Concept — The Four Gates — reveals that every Pulse flip enters a four-gate pipeline that decides whether the structural break qualifies as a tradeable PX signal. Gate 1 (Body Filter): body greater than min_body, asset-routed (Forex/Commodity 0.02, Crypto/Index 0.03, Stocks 0.05 ATR). Gate 2 (Pre-Cross Distance): pre_cross_dist greater than min_dist, asset+TF matrix from 0 to 0.15 ATR. Gate 3 (Chop Suppression): no opposite-direction PX within flip_window (1-5 bars by asset+TF). Gate 4 (Failed-Flip Override): bypasses Gates 2 and 3 for V-bottom and W-bottom recoveries within 8 bars. Master tables for asset routing and timeframe routing. Plain-English clause-by-clause walkthrough of the px_long boolean expression. Reading PX in the Command Center: direction + Strong marker + bars-since-fired + context tag + guidance. PX vs TS preview: PX catches the flip, TS catches the snap before. Strong vs Standard PX: pipeline qualification plus 4-factor conviction overlay (Ribbon/ADX/Volume/Momentum). Three PX playbooks: Trend Continuation, Reversal from Extreme, Failed-Flip V-Bottom. The diagnostic skill: six rejection patterns to identify why a flip did not fire. Earns the Pulse Cross Operator certificate.', estimatedMinutes: 55, isFree: false, totalSections: 17, quizPassThreshold: 66 },
    ],
  },
];
